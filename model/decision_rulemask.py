...
class Module(nn.Module):
    ...
    def __init__(self, args, device='cpu'):
        ...
        # Entailment Tracking
        electra_model_path = './pretrained_models/electra-large-discriminator'
    ...
    def forward(self, batch):
        ...
        tenc_input, tenc_mask, tenc_input_ori = [], [], []
        rule_mask = []
        ...
        for idx, e in enumerate(batch):
            ...
            tenc_input_ori.append(torch.index_select(electra_enc[idx], 0, tenc_idx))
            ...
            tenc_input.append(torch.Tensor([t.cpu().detach().numpy() for t in tenc_input_]))
        ...
        if self.args.trans_layer > 0:
            ...
            tenc_input_ori_padded = torch.nn.utils.rnn.pad_sequence(tenc_input_ori).to(self.device)
            tenc_mask_padded = torch.nn.utils.rnn.pad_sequence(tenc_mask, batch_first=True, padding_value=True).to(self.device)
            tenc_output_ori = self.transformer_encoder(tenc_input_ori_padded, src_key_padding_mask=tenc_mask_padded)
            tenc_out = self.transformer_encoder(tenc_input_padded, src_key_padding_mask=tenc_mask_padded)
            # print(tenc_output_ori.shape)
            # print(tenc_out.shape)
            tenc_out_t = torch.transpose(tenc_out + tenc_output_ori, 0, 1).contiguous()  # [bz * seqlen * dim]
        else:
            tenc_out_t = torch.nn.utils.rnn.pad_sequence(tenc_input + tenc_input_ori, batch_first=True).to(self.device)
    ...
    def run_train(self, train, dev):
        ...
        fh = logging.FileHandler(os.path.join(self.args.dsave, 'train_{}_{}_{}.log'.format(self.args.train_batch, self.args.epoch, self.args.learning_rate)))
